...
#if USE_TIMER_1
Comu zerial(3,4); // pin 3 envia, pin 4 recebe
...
void loop()
{
   byte info = 0x48;  // 0x52 é letra R em ASCII e 0x48 é a letra H

    int status = zerial.envia(info);
    ...
    byte received_byte = zerial.recebe();

    if (received_byte != ocupado) {
        Serial.print("Recebido: ");
        Serial.println((char)received_byte);  // Imprime em char da ASCII
    } else {
        Serial.println("Nenhum byte recebido ou o dispositivo está ocupado.");
    }
